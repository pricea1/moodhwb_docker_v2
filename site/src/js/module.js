...
(function(window, document, $){
	...
	function init(){
		...
		$('.module-nav__row--current').on('click', function(ev){
			ev.preventDefault();
			$(this).parents('.module-nav').toggleClass('module-nav--closed');
		});
		...
		if (typeof opadVideoEl != 'undefined' && $('#' + opadVideoEl).length && opadVideoUrl_hls){
...
//			    primary: "flash"
		...
		doSaveModuleStatus(window.craftSettings.entry.id, "viewed", window.craftSettings.entry.uri);
...
*/
	...
	function initRecommendations(){
		...
		function success(res){
			console.log(res);
			var highlightList = window.craftSettings.user.hereFor == "self"? res.hereForSelf : res.hereForOther;

			for (var i=0; i < highlightList.length; i++){
				$('.module-nav__list--level2 .module-nav__row[data-page-id="' + highlightList[i] + '"]').addClass('module-nav__row--highlight');
			}
