...
function moveTemplates() {
  //Move
  return gulp.src(["./src/templates/**/*"]).pipe(gulp.dest("./templates"));
}
...
function moveFonts() {
  return gulp.src("./src/fonts/**/*").pipe(gulp.dest("./web/resources/fonts"));
}

function minify() {
  gulp
    .src([".src/js/feature-block/**"])
    .pipe(gulp.dest("./web/resources/js/feature-block/"));

  return gulp
    .src(["./src/templates/_layout.html"])
    .pipe(
      usemin({
        assetsDir: "src",
        js: [gulpif(argv.production, uglify()), rev],
...
gulp.task("clean:resources:js", function() {
  return del(["./web/resources/js/**/*.js"]);
});

function moveIndexTemplate() {
  gulp.src(["./web/_layout.html"]).pipe(gulp.dest("./templates"));
...
function watch() {
  ...
  gulp.watch(
    "./src/sass/**/*.scss",
    { delay: 500 },
    gulp.series(compassCompile, minify, moveIndexTemplate)
  );
  ...
  gulp
    ...
    .on("change", function(file) {
      ...
      return gulp.src([file]).pipe(gulp.dest(destFolder));
    });

  gulp.watch(
    ["./src/js/**/*.js", "src/templates/_layout.html"],
    { delay: 500 },
    gulp.series("clean:resources:js", minify, moveIndexTemplate)
  );

  gulp.watch(
    "./src/templates/**/*.hbs",
    { delay: 500 },
    gulp.series(hbtemplatesCompile, minify)
  );
