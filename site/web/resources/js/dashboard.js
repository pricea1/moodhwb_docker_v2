...
(function(window, document, $) {
  ...
  function initMoodList() {
    var moodListContainer = document.getElementById(
        "profileDashboardMoodScrollContainer"
      ),
    ...
    $(moodListEl)
      .width(moodItemsWidth)
      .addClass("profile-dashboard-mood-list--show");
    moodListContainer.scrollLeft =
      (moodListEl.clientWidth - moodListContainer.clientWidth) / 2;
  }
  ...
  function initRecommendations() {
    ...
    function success(res) {
      for (var i = 0; i < res.hereForSelf.length; i++) {
        $("#submodule-" + res.hereForSelf[i] + " a").addClass(
          "profile-dashboard-module-highlight-self"
        );
      }

      for (var i = 0; i < res.hereForOther.length; i++) {
        $("#submodule-" + res.hereForOther[i] + " a").addClass(
          "profile-dashboard-module-highlight-other"
        );
      }
    }
